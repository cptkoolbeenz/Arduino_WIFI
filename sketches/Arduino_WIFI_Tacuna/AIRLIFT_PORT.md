# Arduino_WIFI_Tacuna AirLift Shield Port

Ports the full `Arduino_WIFI_Tacuna.ino` sketch (AD7193 acquisition + SD logging + UDP/TCP command protocol) from the Uno R4 WiFi's onboard ESP32-S3 (`WiFiS3`) to an external Adafruit AirLift Shield #4285 driven over SPI by `WiFiNINA`. The R4 Minima will work too — only the FQBN changes.
...
`START_HOUR`/`END_HOUR` (default 7-19 in Bob's original; widened to 0-23 in this sketch for bench tests) gate WiFi mode. Outside the window the sketch is in pure AD7193 acquisition mode and does not service UDP at all.
...
All PowerShell and stored next to this sketch — see "Windows Firewall" quirk above for why.
